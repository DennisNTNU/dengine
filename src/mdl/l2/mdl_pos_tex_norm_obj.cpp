...
Mdl_pos_tex_norm_obj::Mdl_pos_tex_norm_obj(GLuint shaderID, GLuint textureID, const char* obj_file_path)
    : Mdl_pos_tex_norm(shaderID, textureID)
{
    strncpy(_name, "Mdl_pos_tex_norm_obj", 127);
    ...
    if (ret == 0)
    {
        _initVAO(indices, vertexPositions, vertexUVs, vertexNormals, vertexCount);
        delete[] indices;
...
Mdl_pos_tex_norm_obj::~Mdl_pos_tex_norm_obj()
{

}

/*
void Mdl_pos_tex_norm_obj::initData(unsigned int** indices, float** vertexPositions, float** vertexUVs, int* vertexCount)
{
    *vertexCount = 30;
